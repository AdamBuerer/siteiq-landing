...
**Stakeholder / production landing URL:** In Vercel, set **Production Branch** to **`landing`**. Production runs **`npm run build:home`**, which is **`vite build --mode home`**: the **full** page (all sections) ships, with **`VITE_HOME_ONLY`** so the **nav** is minimal (logo + CTAs, no mega menu / drawer)—not a shortened page.
...
**Preview deployments** (e.g. PRs from `main`): `scripts/vercel-build.mjs` runs the **full** `npm run build` on branches other than `landing` / `home`, so previews use the full nav. Production on `landing` uses the home-only nav styling via env.
